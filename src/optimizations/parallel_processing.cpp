...
void batchInsert(VectorDatabase &db, const std::vector<Vector> &vectors,
  ...
  if (batch_size == 0 &&
      vectors.size() > 0) { // Handle cases with more threads than items
    batch_size = 1;
...
batchSimilaritySearch(VectorDatabase &db, const std::vector<Vector> &queries,
                      size_t k) {
...
void parallel_for_each(std::vector<int> &indices,
...
std::vector<float> parallel_transform(const std::vector<Vector> &queries,
  ...
  auto task = [&](size_t start, size_t end) {
    for (size_t i = start; i < end; ++i) {
      results[i] = std::inner_product(queries[i].begin(), queries[i].end(),
                                      centroid.begin(), 0.0f);
    }
  };
